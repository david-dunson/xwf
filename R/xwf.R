...
#' @param b Parameter of the weighting function. See original paper for details.
...
#' @param left Boolean specifying whether the left (TRUE) or right (FALSE) extrema-weighted features should be computed: Left and right refer to the weighting function.
#' 
...
#' @export
xwf <-
function(xx, t, n.i, psi, b = .5, F, t.min = NULL, t.max = NULL, t.range = NULL, rel.shift = .001, left = TRUE) {
  ...
  return(Vectorize(
    FUN = function(i) integrate(f = function(t) ifelse(left, min(1, (1-F(xx[[i]](t)))/(1-b)), min(1, F(xx[[i]](t))/b))*psi(xx[[i]], t), lower = t.min[i]+shift[i], upper = t.max[i]-shift[i], subdivisions = 100, rel.tol = .05, abs.tol = .05, stop.on.error = FALSE)$value/t.range[i]
  )(1:n))
}